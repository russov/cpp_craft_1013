...
namespace multicast_communication
{
    ...
    { 
        ...
        {
            boost::mutex::scoped_lock lock( protect_trade_queue_ );
            trade_queue_.push( m.data );
        }
        else if(m.type_ == udp_listener::message::QUOTE)
        {
            boost::mutex::scoped_lock lock( protect_quote_queue_ );
    ...
    { 
                ...
                it != trade_ports.end(); ++it)
        {
            udp_listener::udp_listener_ptr c_ptr( 
                    new udp_listener(*it, boost::bind(&market_data_receiver::receive, this, _1), udp_listener::message::TRADE, io_service_ ) );
        ...
        {
            udp_listener::udp_listener_ptr c_ptr( 
                    new udp_listener(*it, boost::bind(&market_data_receiver::receive, this, _1), udp_listener::message::QUOTE, io_service_ ) );
            listeners_.push_back(c_ptr);
            c_ptr->socket_reload_();
            c_ptr->register_listen_();
        }
    ...
    void market_data_receiver::process_trades_()
    {
        while(true)
        {
            ...
            converter<trade_message>::convert_block( s, trades );
    ...
    void market_data_receiver::process_quotes_()
    {
        while(true)
        {
            ...
            converter<quote_message>::convert_block( s, quotes );
