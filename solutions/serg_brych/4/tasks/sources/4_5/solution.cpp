...
{
	uint32_t count_of_cpu = boost::thread::hardware_concurrency();
	int temp_init_value = m_max;
	
	for(uint32_t i = 0; i < count_of_cpu; i++)
	{
		m_threads.create_thread(boost::bind( &solution::process_thread, this, boost::ref(data), boost::ref(temp_init_value)));
	}
	m_threads.join_all();
}
